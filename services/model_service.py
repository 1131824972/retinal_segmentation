"""
AI 模型服务核心模块 (Model Service Core)
---------------------------------------
本模块封装了 PyTorch 模型的加载、预处理、推理和后处理逻辑。
主要职责：
1. 在服务启动时加载 .pt 模型文件到内存/显存。
2. 对输入图像进行 Resize 和 Min-Max 归一化（与训练时保持一致）。
3. 执行推理并处理双通道输出。
4. 将推理结果转换为二值化掩码并编码为 Base64。
"""
import logging
import time
import numpy as np
